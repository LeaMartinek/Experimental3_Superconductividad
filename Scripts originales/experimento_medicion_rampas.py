"""
Programa final para realizar mediciones en la muestra por medio de rampas.
"""
...
"""
La rampa funciona cuando quiere, el problema radica en que
cuando se setea no se setea el punto de inicio en el comando
Encontramos que para que empiece a funcionar suele ser util
setear una rampa manualmente en el equipo.
"""
def rampa(T_fin,paso,tc,nanovolt,fuente,t0,file,axTc,axNv,axNt):
	...
	tc.write('SETP1,'+str(T)+';') # setea el setPoint a la T actual
	#time.sleep(5.)

	print('SRMP1,1,'+str(T_fin)+','+str(paso)+';')

	tc.write('SRMP1,1,'+str(T_fin)+','+str(paso)+';') # rampa, va desde temperatura actual a 80.0 K a 1.0 K/min
...
"""
Seteamos los valores iniciales y realizamos las conecciones a los equipos
"""
...
nombreArchivo = ((time.ctime(time.time()).replace(':',' '))+' .txt').replace(' ','_')
...
"""
Rutina:
El programa esta pensado para comenzar a temperatura ambiente, o al menos mayor a 100 K.
